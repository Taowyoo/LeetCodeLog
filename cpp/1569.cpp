...
public:
    int numOfWays(vector<int>& nums) {
        ...
        long long ans = dfs(nums, 0, mod);
        return ans % mod - 1;
    }
    
private:
    vector<vector<long long>> table;
    long long dfs(vector<int> &nums, int idx, long long mod){
        int n = nums.size();
        if(n <= 2) return 1;
        ...
        long long left_res = dfs(left, 0, mod) % mod;
        long long right_res = dfs(right, 0, mod) % mod;
