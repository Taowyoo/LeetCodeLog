// dp with memory + presum

// 1. We can simply do what's stated in the question and try all the possible partitions.
// 2. If the sum of left row is less recur on the left row.
// 3. If the sum of right row is less recur on the right row.
// 4. If the sum of both rows are equal we try recuring on both the partitions and chose the one with maximum result.
// 5. Now simply repeat the steps for the new row
// 6. Do this until there is only 1 stone left.
// 7. We can see that there are recalculations of selected row when we perform the previous 3 steps. Thus we memoize on the row represented by i,j i.e row start and row end.
// Time : O(n^3)
// Space : O(n ^ 2)
class Solution {
public:
